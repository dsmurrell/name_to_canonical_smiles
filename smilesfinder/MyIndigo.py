...
class MyIndigo:
    ...
    def fetchCanonicalSMILESbyNameFast(self, molecule_name):
        ...
        response = urllib2.urlopen('http://cactus.nci.nih.gov/chemical/structure/' + URLFetchingHelpers.url_fix(myfix) + '/stdinchikey')
    ...
    def fetchCanonicalSMILESbyName(self, molecule_name):
        ...
        myfix = myfix.decode('utf-8')
        myfix = myfix.replace(u'«', '\'')
        response = urllib2.urlopen('http://cactus.nci.nih.gov/chemical/structure/' + URLFetchingHelpers.url_fix(myfix) + '/smiles/xmls?resolver=name_by_chemspider,name_by_opsin,name_by_cir')
        text = response.read()
